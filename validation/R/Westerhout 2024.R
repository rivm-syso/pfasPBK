...
Background_PFAS_amounts <- read.csv2(file = 'validation/R/Westerhout 2024 - EFSA TWI background amounts.csv',na.strings=c(""," ","NA"),stringsAsFactors = FALSE)
...
output.df <- Variables_df %>%
  rename(time = TIME) %>%
  left_join(output.df)
write.csv(output.df, './validation/R/results/validation_result.csv')
