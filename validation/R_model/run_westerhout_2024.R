...
# Code changed to represent current (07-11-2025) antimony implementation 
# With lifetime equation, but starting at 0 and no exposure via breastfeeding
...
TSTART <- 0
TSTOP <- 365*80 # days
exposure_duration = 365*80 # Duration of exposure (d)
DT <- 1
TIME <- seq(TSTART,TSTOP,by=DT)

Oralexpo_PFOA = 0.001 # 0.000187 # µg/kg/day [EFSA opinion 2020, p.144] # 0.00015 = TWI of 4.4 ng/kg/wk assuming PFOA:PFOS ratio of 24:76 from [EFSA opinion 2020, p.152]
Oralexpo_PFOS = 0.001 # 0.000444 # µg/kg/day [EFSA opinion 2020, p.144] # 0.000458 = TWI of 4.4 ng/kg/wk assuming PFOA:PFOS ratio of 24:76 from [EFSA opinion 2020, p.152]
...
fss <- 0.05 # fraction palm of hands (Sheridan et al., 1995, Rhodes et al., 2013)
...
# Oralexpo_PFOA = 0.000187 # µg/kg/day [EFSA opinion 2020, p.144] # 0.00015 = TWI of 4.4 ng/kg/wk assuming PFOA:PFOS ratio of 24:76 from [EFSA opinion 2020, p.152]
Drinkconc_PFOA = 0 # Drinking water concentration (ug/L or ppb) 

# Inhalation exposure
Cinh_PFOA = 0 # 0.000526 # ug/L; 0.526 ug/m3 [Nilsson 2013]
Cinh_82FTOH = 0 # 0.114 # ug/L; 114 ug/m3 [Nilsson 2013]
...
# Dermal exposure
Cdermal_PFOA = 0 # 680 # ug/kg = 0.680 ug/g [Freberg 2010]
...
# PFOA
# PFOAmaternal = 0 # 2.0 # maternal concentration ng/mL at delivery [EFSA opinion 2020, p368]
...
# Aartbirth_PFOA = 0.39*APlasbirth_PFOA # initial amount in arterial plasma at birth
# Avenbirth_PFOA = 0.61*APlasbirth_PFOA # initial amount in venous plasma at birth
...
# ALunbirth_PFOA = APlasbirth_PFOA*4.52 # initial amount in lung tissue at birth (manually estimated)
# ASkbirth_PFOA = APlasbirth_PFOA*3.39 # initial amount in skin tissue at birth (manually estimated)
...
# Oral exposure via the mother
# Oralexpo_PFOS = 0.000444 # µg/kg/day [EFSA opinion 2020, p.144] # 0.000458 = TWI of 4.4 ng/kg/wk assuming PFOA:PFOS ratio of 24:76 from [EFSA opinion 2020, p.152]
...
# Aartbirth_PFOS = 0.39*APlasbirth_PFOS # initial amount in arterial plasma at birth
# Avenbirth_PFOS = 0.61*APlasbirth_PFOS # initial amount in venous plasma at birth
...
# ALunbirth_PFOS = APlasbirth_PFOS*3.73 # initial amount in lung tissue at birth (manually estimated)
# ASkbirth_PFOS = APlasbirth_PFOS*8.71 # initial amount in skin tissue at birth (manually estimated)
...
Variables_df <- Variables_df %>%
  ...
  mutate(Oraldose_PFOS = if_else(day > exposure_duration,0,Oralexpo_PFOS*BW)) %>% # stop exposure after exposure_duration
  ...
  # Inhalation exposure
  # Occupational (ski wax technician) inhalation exposure is assumed to take place 5 days per week, 6 months per year
  ...
  # mutate(Inhalation_PFOA = if_else(age >= age_start_occ_exp & age <= age_stop_occ_exp,Inhalation_PFOA,0)) %>% # stop exposure after exposure_duration
  ...
  # mutate(Inhalation_PFOS = if_else(age >= age_start_occ_exp & age <= age_stop_occ_exp,Inhalation_PFOS,0)) %>% # stop exposure after exposure_duration
  
  # Dermal exposure
  # Occupational (ski wax technician) dermal exposure is assumed to take place 5 days per week, 6 months per year
  mutate(Csurf_PFOA = 0) %>%
  # mutate(Csurf_PFOA = if_else(dayoftheweek >= 1 & dayoftheweek <= 5,Cdermal_PFOA_total_day,Csurf_PFOA)) %>% # stop exposure after exposure_duration
  # mutate(Csurf_PFOA = if_else(month >= 5 & month <= 10,0,Csurf_PFOA)) %>% # stop exposure after exposure_duration
  # mutate(Csurf_PFOA = if_else(age >= age_start_occ_exp & age <= age_stop_occ_exp,Csurf_PFOA,0)) %>% # stop exposure after exposure_duration
  mutate(Csurf_PFOS = 0) %>%
  # mutate(Csurf_PFOS = if_else(dayoftheweek >= 1 & dayoftheweek <= 5,Cdermal_PFOS_total_day,Csurf_PFOS)) %>% # stop exposure after exposure_duration
  # mutate(Csurf_PFOS = if_else(month >= 5 & month <= 10,0,Csurf_PFOS)) %>% # stop exposure after exposure_duration
  # mutate(Csurf_PFOS = if_else(age >= age_start_occ_exp & age <= age_stop_occ_exp,Csurf_PFOS,0)) %>% # stop exposure after exposure_duration 
  ...
  # mutate(VSk = (SkinTarea*Skinthickness)/1000) %>% # Skin volume (L) # original = all skin
  mutate(VSk = (fss*SkinTarea*Skinthickness)/1000) %>% # Skin volume (L) # new (07-11-2025) = only exposed skin
...
PFAS_extended <- function(t, A, parms) {
  with(as.list(c(A, parms)), {
    ...
    # Clun_bl_PFOA <- Alun_PFOA/(Valv*Pab_PFOA + Vlun) # original
    # Calv_PFOA <- (Alun_PFOA/(Valv*Pab_PFOA + Vlun))*Pab_PFOA # original
    Clun_PFOA <- Alun_PFOA/Vlun # new (07-11-2025)
    ...
    # Clun_bl_PFOS <- Alun_PFOS/(Valv*Pab_PFOS + Vlun) # original
    # Calv_PFOS <- (Alun_PFOS/(Valv*Pab_PFOS + Vlun))*Pab_PFOS # original
    Clun_PFOS <- Alun_PFOS/Vlun # new (07-11-2025)
    ...
    dASk_PFOA <- QSk*(Cart_PFOA*Free_PFOA-CSk_PFOA*FreeSk_PFOA)    # new (07-11-2025)
    
    # dASk_PFOS <- CLcell*Cve_PFOS/Kver_PFOS + QSk*(Cart_PFOS*Free_PFOS-CSk_PFOS*FreeSk_PFOS)    # Rate of change in skin(µg/h) 
    dASk_PFOS <- QSk*(Cart_PFOS*Free_PFOS-CSk_PFOS*FreeSk_PFOS)    # new (07-11-2025)
    ...
    list(c(dAlun_PFOA, dASk_PFOA, dAven_PFOA, dAart_PFOA, # dAsc_PFOA, dAtrans_PFOA, dAve_PFOA, 
           dAG_PFOA, dAL_PFOA, dAF_PFOA, dAK_PFOA, dAfil_PFOA, dAdelay_PFOA, dAurine_PFOA, dAR_PFOA,
           dAlun_PFOS, dASk_PFOS, dAven_PFOS, dAart_PFOS, # dAsc_PFOS, dAtrans_PFOS, dAve_PFOS, 
           dAG_PFOS, dAL_PFOS, dAF_PFOS, dAK_PFOS, dAfil_PFOS, dAdelay_PFOS, dAurine_PFOS, dAR_PFOS),
         c(Cart_PFOA=Cart_PFOA, Cven_PFOA=Cven_PFOA, Clun_PFOA=Clun_PFOA, # Clun_bl_PFOA=Clun_bl_PFOA,
           CSk_PFOA=CSk_PFOA, # Csurf_PFOA=Csurf_PFOA, Csc_PFOA=Csc_PFOA,Ctrans_PFOA=Ctrans_PFOA,Cve_PFOA=Cve_PFOA, 
           ...
           Cart_PFOS=Cart_PFOS, Cven_PFOS=Cven_PFOS, Clun_PFOS=Clun_PFOS, # Clun_bl_PFOS=Clun_bl_PFOS,
           CSk_PFOS=CSk_PFOS, # Csurf_PFOS=Csurf_PFOS, Csc_PFOS=Csc_PFOS,Ctrans_PFOS=Ctrans_PFOS, Cve_PFOS=Cve_PFOS, 
...
#### A_init ----
# Initial values
A_init_PFAS_extended <- c(Alun_PFOA = 0, # ALunbirth_PFOA,
                          # Asc_PFOA = 0,
                          # Atrans_PFOA = 0,
                          # Ave_PFOA = 0,
                          ASk_PFOA = 0, # ASkbirth_PFOA,
                          Aven_PFOA = 0, # Avenbirth_PFOA,
                          Aart_PFOA = 0, # Aartbirth_PFOA,
                          AG_PFOA = 0, # AGbirth_PFOA,
                          AL_PFOA = 0, # ALbirth_PFOA, # Initial amount in liver (ug)
                          AF_PFOA = 0, # AFbirth_PFOA,
                          AK_PFOA = 0, # AKbirth_PFOA,
                          Afil_PFOA = 0, # Afilbirth_PFOA,
                          Adelay_PFOA = 0, # Adelaybirth_PFOA,
                          Aurine_PFOA = 0, # Aurinebirth_PFOA,
                          AR_PFOA = 0, # ARbirth_PFOA,
                          Alun_PFOS = 0, # ALunbirth_PFOS,
                          # Asc_PFOS = 0,
                          # Atrans_PFOS = 0,
                          # Ave_PFOS = 0,
                          ASk_PFOS = 0, # ASkbirth_PFOS,
                          Aven_PFOS = 0, # Avenbirth_PFOS,
                          Aart_PFOS = 0, # Aartbirth_PFOS,
                          AG_PFOS = 0, # AGbirth_PFOS,
                          AL_PFOS = 0, # ALbirth_PFOS, # Initial amount in liver (ug)
                          AF_PFOS = 0, # AFbirth_PFOS,
                          AK_PFOS = 0, # AKbirth_PFOS,
                          Afil_PFOS = 0, # Afilbirth_PFOS,
                          Adelay_PFOS = 0, # Adelaybirth_PFOS,
                          Aurine_PFOS = 0, # Aurinebirth_PFOS,
                          AR_PFOS = 0) # ARbirth_PFOS)
...
write.table(output.df, file = "validation/results/results_R_PFAS.csv", dec = ".", sep = ",", row.names = FALSE, na = "")
